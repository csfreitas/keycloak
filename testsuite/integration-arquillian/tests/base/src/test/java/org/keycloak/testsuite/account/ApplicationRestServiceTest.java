...
public class ApplicationRestServiceTest extends AbstractRestServiceTest {
    ...
    public void configureTestRealm(RealmRepresentation testRealm) {
        ...
        testRealm.getUsers().add(UserBuilder.create().username("alice").password("password").enabled(true)
                .role("account", AccountRoles.MANAGE_ACCOUNT).build());
    }

    @Test
    public void testGetApplications() throws IOException {
        oauth.doLoginGrant("admin", "admin");

        oauth.clientId("account");

        OAuthClient.AccessTokenResponse accessToken = oauth
                .doAccessTokenRequest(oauth.getCurrentQuery().get(OAuth2Constants.CODE), "password");
        List<ApplicationResource.ApplicationRepresentation> sessions = SimpleHttp
                .doGet(getAccountUrl("applications"), httpClient).auth(accessToken.getAccessToken())
