...
public class CookiesPathTest extends AbstractKeycloakTest {
    ...
    public void testMultipleCookies() throws IOException {
        ...
        Assert.assertThat(cookieStore.getCookies().stream().map(c -> c.getName()).collect(Collectors.toList()), 
                Matchers.containsInAnyOrder("AUTH_SESSION_ID", "KEYCLOAK_IDENTITY", "KEYCLOAK_SESSION"));

        // does each cookie's path end with "/"
        cookieStore.getCookies().stream().map(c -> c.getPath()).forEach(path ->Assert.assertThat(path, Matchers.endsWith("/")));
    ...
    public void testOldCookieWithWrongPath() {
        ...
        // does each cookie's path end with "/"
        cookies.stream().map(c -> c.getPath()).forEach(path -> Assert.assertThat(path, Matchers.endsWith("/")));
